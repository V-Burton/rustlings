...
fn build_scores_table(results: String) -> HashMap<String, Team> {
    ...
    for r in results.lines() {
        ...
        //Two different option here with a preference for the first one since `get_mut` is called only once and it is easier to read. 
        if let Some(scores) = scores.get_mut(&team_1_name){
